import { MessageType, sendTabMessage } from '../shared/messaging';
...
function getChatGPTContentScriptFiles(): string[] {
  ...
  const entry = scripts.find((cs) =>
    cs.matches?.some((m) => m.includes('chatgpt.com') || m.includes('chat.openai.com')),
  );
  return entry?.js ?? [];
}

export async function findChatGPTTabId(): Promise<number | null> {
...
export async function getOrCreateChatGPTTab(): Promise<number> {
  ...
  if (tab.id == null) throw new Error('Không tạo được tab ChatGPT');
  return tab.id;
}
...
export async function ensureChatGPTContentScript(tabId: number): Promise<void> {
  try {
    const res = await sendTabMessage<{ ok?: boolean }>(tabId, {
  ...
  if (
    ...
    throw new Error(
      'Tab ChatGPT chưa sẵn sàng — mở https://chatgpt.com rồi thử lại',
    );
  }

  const files = getChatGPTContentScriptFiles();
  if (files.length === 0) {
    throw new Error('Extension thiếu content script ChatGPT — build lại và reload');
  }
...
export async function sendChatGPTImagePrompt(
  tabId: number,
  prompt: string,
): Promise<void> {
  const message = {
  ...
  for (let i = 0; i < 30; i++) {
    if (i === 0 || (i % 3 === 0 && !injected)) {
      try {
        ...
        if (!isNoReceiverError(err) && i > 4) throw err;
      }
    }

    try {
      const res = await sendTabMessage<{ ok: boolean; error?: string }>(
        tabId,
        message,
      );
      if (res?.ok) return;
      ...
      if (isNoReceiverError(err)) injected = false;
    }
    await sleep(500);
  }

  throw new Error(
    `${lastError}. Hãy mở https://chatgpt.com, F5 trang rồi bấm tạo ảnh lại.`,
  );
}

export async function openChatGPTWithImagePrompt(prompt: string): Promise<void> {
