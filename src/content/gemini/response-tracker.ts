import {
  getGeminiLastResponseHash,
  parseGeminiProductJson,
} from '../../shared/storage';
import { hashText } from '../../shared/text-hash';
import { getModelResponseElements, isGeminiGenerating } from './dom-query';
import {
  buildBubbleRows,
  ...
  geminiDebugTable,
  probeSelectorCounts,
} from './gemini-debug-log';

/** Băm mỗi 1 giây; 2 lần liên tiếp trùng = stream xong */
const POLL_INTERVAL_MS = 1000;
const STABLE_POLLS_REQUIRED = 2;

export interface ResponseSendSnapshot {
  modelCount: number;
  baselineHash: string;
  previousResponseHash: string | null;
  scopedRoot: ParentNode;
  sourceContentHash?: string;
}

export interface StableResponseResult {
  text: string;
  hash: string;
  elementIndex: number;
}

let activeWaitCancel: (() => void) | null = null;
let pollTickCount = 0;

export function cancelActiveGeminiWait(): void {
  activeWaitCancel?.();
  activeWaitCancel = null;
}

function getScope(_snapshot: ResponseSendSnapshot): ParentNode {
  return document;
}

function readBubbleText(el: HTMLElement | undefined): string {
  if (!el) return '';
  return (el.innerText ?? el.textContent ?? '').trim();
}

function pickWatchBubble(
  nodes: HTMLElement[],
  snapshot: ResponseSendSnapshot,
): { el: HTMLElement; index: number; strategy: string } | null {
...
function hasValidProductJson(text: string): boolean {
  return !!parseGeminiProductJson(text);
}

function isStreamingSettled(stableStreak: number): boolean {
  return stableStreak >= STABLE_POLLS_REQUIRED;
}

function isNewResponseHash(
  currentHash: string,
  snapshot: ResponseSendSnapshot,
...
function finish(
  resolve: (r: StableResponseResult) => void,
  text: string,
  index: number,
): void {
  resolve({
...
function refreshWatchCache(
  ...
  if (forceScan && cache.nodeLength !== nodes.length) {
    ...
    geminiDebugTable(
      'scan',
      `Bubbles document (modelCount=${snapshot.modelCount}, hiện=${nodes.length})`,
      buildBubbleRows(nodes),
      picked?.index,
    );
  }
...
export async function captureSendSnapshot(
  sourceContentHash?: string,
): Promise<ResponseSendSnapshot> {
  geminiDebugClearPanel();
  ...
  const lastText =
    modelCount > 0 ? readBubbleText(docNodes[modelCount - 1]) : '';
  ...
  if (modelCount > 0) {
    geminiDebugTable(
      'snapshot',
      `Bubble document — theo dõi index=${modelCount} khi có câu mới`,
      buildBubbleRows(docNodes),
      modelCount - 1,
    );
  }
...
export function waitForNewStableResponse(
  snapshot: ResponseSendSnapshot,
  timeoutMs = 120000,
): Promise<StableResponseResult> {
  pollTickCount = 0;
  ...
  return new Promise((resolve, reject) => {
    let lastPollHash: string | null = null;
    let stableStreak = 0;
    let lastSeenText = '';
    let lastIndex = -1;
    let cancelled = false;
    const cache: WatchCache = { bubble: null, bubbleIndex: -1, nodeLength: -1 };
    ...
    const succeed = (text: string, index: number) => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      fail(
        new Error(
          `Hết thời gian chờ phản hồi Gemini${hint} — kiểm tra tab Gemini có JSON title/description`,
        ),
      );
    }, timeoutMs);
    ...
    const tick = () => {
      ...
      const awaitingMoreNodes =
        generating && cache.nodeLength <= snapshot.modelCount;
      const forceScan =
        !cache.bubble?.isConnected ||
        cache.nodeLength < 0 ||
        awaitingMoreNodes ||
      ...
      const picked = refreshWatchCache(
        scope,
        snapshot,
        cache,
        forceScan,
      ...
      if (!settled) blockers.push(`ổn định ${stableStreak}/${STABLE_POLLS_REQUIRED}`);
      if (!hashCheck.ok) blockers.push(hashCheck.reason!);
      if (!jsonOk) blockers.push('chưa parse được JSON title/description');
      ...
      if (!settled || !hashCheck.ok || !jsonOk) return;
