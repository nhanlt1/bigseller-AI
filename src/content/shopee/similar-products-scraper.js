...
export function parsePriceRange(text) {
    ...
    if (!raw)
        return { priceText: '', priceMin: null, priceMax: null };
    ...
    if (parts.length >= 2) {
        ...
        const priceText =
            priceMin && priceMax
                ...
                : raw;
        return { priceText, priceMin: priceMin || null, priceMax: priceMax || null };
    }
...
export function parseSoldCount(text) {
    ...
    if (!raw)
        return { soldText: '', soldNumeric: null };
    const soldText = raw;
...
function readCardPrice(card) {
    ...
    const currency = parent?.querySelector('span:not(.truncate)')?.textContent?.trim();
    if (text && currency === '₫' && !text.includes('₫'))
        text = text + '₫';
    if (!text) {
...
export function scrapeMainProduct(root = document, page = 1) {
    ...
    const soldEl = block.querySelector('.shopee-product-info__header__sold-count');
    const priceEl = block.querySelector('.FZKPyi');
    const ratingBlock = block.querySelector('.shopee-product-info__header__rating');
...
function formatRatingDisplay(product) {
    ...
    const count =
        product.ratingCount ??
        parseRatingCount(product.ratingCountText ?? '');
    if (score != null && count)
        return `${score} · ${count}`;
    if (score != null)
        return String(score);
    if (product.ratingCountText)
        return product.ratingCountText;
    if (count)
        return String(count);
    return '';
}
