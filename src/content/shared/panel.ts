...
export class FloatingPanel {
  ...
  private titlePreviewEl!: HTMLElement;
  private descPreviewEl!: HTMLElement;
  private rewriteBtn!: HTMLButtonElement;
  private copyPromptBtn!: HTMLButtonElement;
  private applyBtn!: HTMLButtonElement;

  private lastResult: ProductData | null = null;
  private waitingGemini = false;
  ...
  private render(): void {
    ...
    this.rewriteBtn = this.shadow.getElementById('rewrite-btn') as HTMLButtonElement;
    this.copyPromptBtn = this.shadow.getElementById(
      'copy-prompt-btn',
    ) as HTMLButtonElement;
    this.applyBtn = this.shadow.getElementById('apply-btn') as HTMLButtonElement;
  ...
  private async handleRewrite(): Promise<void> {
    ...
    try {
      ...
      const result = await sendMessage<RewriteResultPayload>({
      ...
      if (generation !== this.rewriteGeneration) return;
  ...
  private async handleCopyPrompt(): Promise<void> {
    ...
    try {
      ...
      const tabResult = await sendMessage<OpenGeminiTabResult>({
      ...
      this.setState(
        'done',
        'Đã copy prompt — dán vào Gemini, copy JSON phản hồi rồi bấm Áp dụng vào form',
      );
    } catch (err) {
      this.setState(
        'error',
        err instanceof Error ? err.message : 'Không copy được prompt',
      );
    }
  }
  ...
  private async handleApply(): Promise<void> {
    ...
    try {
      ...
      if (!parsed) {
        throw new Error(
          'Clipboard không có JSON {"title":"...","description":"..."} — copy lại phản hồi từ Gemini',
        );
      }
      ...
      this.setState(
        'error',
        err instanceof Error ? err.message : 'Không áp dụng được',
      );
    }
  }
...
export function mountToggleButton(panel: FloatingPanel): void {
