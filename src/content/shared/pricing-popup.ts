import { formatVnd } from '../../pricing/formula-engine';
import {
  evaluateOrderProfit,
  solveMinUnitPrice,
} from '../../pricing/order-profit';
import type { PlatformFeeConfig } from '../../pricing/platform-fee-config';
import { evaluateTiers } from '../../pricing/wholesale-tiers';
...
export class PricingPopup {
  ...
  toggle(): void {
    ...
    if (this.visible) void this.loadAndRender();
  }
  ...
  private readForm(): void {
    ...
    const num = (id: string) =>
      Number((this.shadow.getElementById(id) as HTMLInputElement)?.value) || 0;
    ...
    fee.useVoucherXtra =
      (this.shadow.getElementById('use-voucher-xtra') as HTMLInputElement)
        ?.checked ?? false;
    fee.infrastructureFeePerOrder = num('infra');
    fee.usePiShip =
      (this.shadow.getElementById('use-piship') as HTMLInputElement)?.checked ??
      false;
  ...
  private updateResults(): void {
    ...
    const retailPrice = solveMinUnitPrice(
      1,
      c.costPerUnit,
      c.desiredProfitPerUnit,
      f,
    );
...
let popupInstance: PricingPopup | null = null;

export function getPricingPopup(): PricingPopup {
  if (!popupInstance) popupInstance = new PricingPopup();
  return popupInstance;
}

export function mountPricingFab(): void {
